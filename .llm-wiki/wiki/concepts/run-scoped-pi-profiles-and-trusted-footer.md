...
The same controller-owned footer extension follows the wiki extension in every process. In Herdr TUI mode it reads the `llm-wiki` and `llm-wiki-model` status map through the Pi footer API and replaces the stock/separate wiki presentation with the personal complementary one-line model/thinking/wiki/state/access/context/cost footer; routine activity renders as `🧠 <count-or-dash> · <model-id>` without a provider prefix, while provider-qualified configuration/runtime identities and the values' theme colors remain intact. Typed and generated truncation follows the pinned pi-tui terminal-column tables and grapheme rules for CJK/fullwidth text, combining marks, and emoji. It also changes only the known healthy `<wiki_status>` capability block in the prompt. Unknown or diagnostic blocks and all warning/error/extension-error/stderr/protocol signals remain complete and observable. A concurrent auth-key temporary handoff is accepted only after the authenticated final key is revalidated; replaced construction objects remain identity-checked and fail closed. The launch gate exercises the same generated extension in a real Pi TUI pseudo-terminal and rejects loader diagnostics before accepting the compact marker.
