...
`squire telemetry RUN-ID [--json] [--config FILE]` is a single-run host-only artifact reader without external adapters or mutable-session inspection. Legacy/missing artifacts explicitly report unavailable/incomplete; unsafe artifacts fail with sanitized errors. Terminal accounting warnings do not relabel accepted workflow evidence. The typed reader/validator and versioned artifact are consumed by the separate [historical telemetry cohort workflow](/concepts/historical-telemetry-cohorts.md), which adds provisional backfill, explicit-run comparisons and authenticated import-only CI/merge claims without changing current-run authority. Squire completion is not a merge or exact-head CI claim.
