...
`squire telemetry RUN-ID [--json] [--config FILE]` is a single-run host-only artifact reader without external adapters or mutable-session inspection. Legacy/missing artifacts explicitly report unavailable/incomplete; unsafe artifacts fail with sanitized errors. Terminal accounting warnings do not relabel accepted workflow evidence. The typed reader/validator and versioned artifact remain the preferred current-run authority. [Historical cohorts](historical-telemetry-cohorts.md) use a separate provisional session-import authority and independently authenticated CI/merge evidence; they never replace or rewrite this ledger. Squire completion is not a merge or exact-head CI claim.
