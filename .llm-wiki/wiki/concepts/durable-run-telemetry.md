...
Transient launch retries use distinct session UUIDs under the same logical phase attempt. The state generation ledger links both accounting rows to their immutable input binding; the retry row uses trigger `retry`. Inventory requires a row for every reserved generation, not exactly one row per attempt. An unlaunched reservation or failed pre-event invocation remains incomplete/unknown rather than free. Accepted Plan/Implement sessions are not repeated in recovery comparisons. Backoff is controller time recorded in the generation ledger, not invented model cost or token usage. See [Transient phase launch retry](transient-phase-launch-retry.md).
