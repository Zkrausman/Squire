...
The personal single-ticket controller adds a separately bounded [report-only format correction](report-only-format-correction.md) boundary for eligible Implement extra-details fields. This does not weaken strict acceptance or repair historical failures. Before correction or acceptance, the controller independently reads every report and observation artifact and binds actual bytes/content, length and SHA-256; adapter metadata alone is never evidence validation. Candidate and cumulative wiki checks remain independent of report formatting.
