...
Its bundled Node helper is the only report validator and writes one closed JSON
object with `version`, `id`, `createdAt`, `sessionId`, `reason`, and `context`.
The session binding comes only from `PI_SESSION_ID`; the helper creates the ID
and timestamp, applies the documented reason/context bounds, and refuses
