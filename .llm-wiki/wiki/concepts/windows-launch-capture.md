...
Supported Node is 24 only (`>=24 <25`) on every platform; see [runtime policy](node-24-runtime-policy.md). Windows builds require existing Python/MSVC/Windows SDK and the locally built addon. Node 24 runs bounded Windows CI with the full native regression inventory. Missing support is an actionable error, never a mode-bit or path-only fallback. POSIX does not build/load it. Choose fresh dedicated destinations under validated safe ancestors; common TEMP or data-drive ACLs can permit extra principals to mutate ancestors. Do not weaken production policy to make fixtures pass.
