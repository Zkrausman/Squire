...
Production supports Node 24 only (`>=24 <25`) on every platform; governed Node 22 build/test/import compatibility is bootstrap-only. See [runtime and gate policy](node-24-runtime-and-gate-policy.md). Windows builds require existing Python/MSVC/Windows SDK and the locally built addon. Only Node 24 runs bounded Windows CI; native/ACL/launch/security coverage and timeouts remain mandatory. Missing support is an actionable error, never a mode-bit or path-only fallback. POSIX does not build/load it. Choose fresh dedicated destinations under validated safe ancestors; common TEMP or data-drive ACLs can permit extra principals to mutate ancestors. Do not weaken production policy to make fixtures pass.
