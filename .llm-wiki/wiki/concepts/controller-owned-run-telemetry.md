...
Pi 0.73.1 may emit threshold `compaction_start`/`compaction_end` **after** `agent_end`. Full-stream and overflow transport share terminal selection: preserve the exact terminal assistant event across a complete validated threshold pair with `willRetry: false`, never substitute compaction summary text. Unknown/malformed/truncated trailers, new work or retry intent invalidate the prior selection; an explicit empty transport selection prevents fallback to an obsolete terminal in an accounting prefix. Compaction model usage is not emitted as authoritative assistant usage, so any compaction marks session tokens incomplete (`unsupported_compaction`) without relabelling a valid completed phase report.
