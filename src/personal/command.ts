import { preSessionProcessFailure } from "./provider-launch-failure.js";
import { PhaseExecutionError, type ExecutionFailure } from "./execution-failure.js";
import { execFile } from "node:child_process";

export interface CommandRequest {
  /** Set only by the phase adapter for its Pi invocation, never controller commands. */
  readonly phaseLaunchProvider?: string;
  ...
  async run(request: CommandRequest, signal?: AbortSignal): Promise<CommandResult> {
    return await new Promise<CommandResult>((resolve, reject) => {
      const child = execFile(request.command, [...request.args], {
        ...
        if (request.phaseLaunchProvider) {
          const transient = preSessionProcessFailure(request.phaseLaunchProvider, stdout, stderr, failure.code, signal?.aborted === true || !!(failure as Error & { killed?: boolean }).killed);
