...
export async function captureLaunchMaterial(loaded: LoadedPersonalMvpConfig): Promise<LaunchMaterial> {
  ...
  const config = deepFreeze({ ...clone, launchRetryPolicy: validateLaunchRetryPolicy(clone.launchRetryPolicy), promptPolicy: validatePromptSelection(clone.promptPolicy ?? DEFAULT_PROMPT_SELECTION) });
...
export function validateLaunchMaterial(value: unknown): LaunchMaterial {
  ...
  if (canonical(validateLaunchRetryPolicy(raw.launchRetryPolicy)) !== canonical(validateLaunchRetryPolicy(config.launchRetryPolicy))) throw new Error("captured launch retry policy mismatch");
