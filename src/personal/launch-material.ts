import { validateLaunchRetries } from "./launch-retry.js";
...
export function validateLaunchMaterial(value: unknown): LaunchMaterial {
  ...
  if (validateLaunchRetries(raw.launchRetries) !== validateLaunchRetries(config.launchRetries)) throw new Error("captured launch retry policy mismatch");
...
function assertMaterialState(material: LaunchMaterial, state: PersonalRunState): void {
  ...
  if (validateLaunchRetries(c.launchRetries) !== validateLaunchRetries(state.launchRetries)) throw new Error("launch retry policy mismatch");
...
function validateCapturedConfig(value: unknown): PersonalMvpConfig {
  const c = record(value, ["repository", "dataDirectory", "paths", "linear", "github", "sandbox", "modelPolicy", "escalationPolicy", "reportCorrectionPolicy", "launchRetries", "promptPolicy", "testCommands", "phaseTimeoutMs"], "captured configuration");
  ...
  validateLaunchRetries(c["launchRetries"]);
