import { launchSessionMatches } from "./launch-retry.js";
  ...
  for (const transition of state.stagedTransitions as readonly StagedTransition[]) {
    ...
    if (kind === "reserved") {
      ...
      if (result) {
        ...
        if (result.runId !== state.runId || !launchSessionMatches(state, result) || result.attempt !== selection.attempt || !isDeepStrictEqual(result.profile, selection.profile) || sessions.has(result.sessionId)) throw new Error("staged result provenance mismatch");
