  ...
  for (const transition of state.stagedTransitions as readonly StagedTransition[]) {
    ...
    if (kind === "reserved") {
      ...
      if (result) {
        ...
        if (result.runId !== state.runId || result.sessionFile !== (state.launchGenerations?.find(r => r.sessionId === result.sessionId)?.sessionFile ?? `/ticket/sessions/${selection.phase}/${selection.attempt}.jsonl`) || result.attempt !== selection.attempt || !isDeepStrictEqual(result.profile, selection.profile) || sessions.has(result.sessionId)) throw new Error("staged result provenance mismatch");
