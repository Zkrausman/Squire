...
export function eligibleCorrection(result: VerifyPhaseResult): boolean {
  ...
  return !/\b(?:owner|approv\w*|authoriz\w*|consent|signoff|amend\w*|policy|permissions?|authority ambiguity|security ambiguity|(?:contract|scope) (?:change|amendment|expansion|revision|alteration)|(?:contract|scope) (?:needs?|requires?|must|should|has to|is required to)\b[^.!?\n]{0,50}\b(?:revision|change|amendment|expansion|approv\w*|decision|updat\w*)|(?:outside|beyond) (?:the )?(?:ticket|contract|scope)|requires? (?:owner|human|host) (?:approval|decision|authorization)|(?:ask|request|need|require|obtain|seek|await|wait for|get)\b[^.!?\n]{0,100}\b(?:owner|human|host)\b[^.!?\n]{0,100}\b(?:approv\w*|authoriz\w*|decision|consent|permission|signoff|contract|policy|scope)|(?:change|expand|weaken|alter|revise) (?:the )?(?:ticket|contract|policy|scope)|(?:grant|expose|retrieve) (?:host )?(?:credentials?|secrets?|permissions?))\b/iu.test(feedback);
}
