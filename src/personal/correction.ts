...
export function eligibleCorrection(result: VerifyPhaseResult): boolean {
  ...
  return !/\b(?:owner|approv\w*|authoriz\w*|consent|signoff|amend\w*|contract|policy|permissions?|authority ambiguity|security ambiguity|scope (?:change|amendment|expansion)|(?:outside|beyond) (?:the )?(?:ticket|contract|scope)|requires? (?:owner|human|host) (?:approval|decision|authorization)|(?:ask|request|need|require|obtain|seek|await|wait for|get)\b[^.!?\n]{0,100}\b(?:owner|human|host)\b[^.!?\n]{0,100}\b(?:approv\w*|authoriz\w*|decision|consent|permission|signoff|contract|policy|scope)|(?:change|expand|weaken) (?:the )?(?:ticket|contract|policy|scope)|(?:grant|expose|retrieve) (?:host )?(?:credentials?|secrets?|permissions?))\b/iu.test(feedback);
}
