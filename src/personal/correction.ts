...
export function eligibleCorrection(result: VerifyPhaseResult): boolean {
  ...
  return !/\b(?:owner|stakeholders?|decisions?|approv\w*|authoriz\w*|consent|signoff|amend\w*|policy|permissions?|authority ambiguity|security ambiguity|(?:contract|scope)\b[^.!?\n]{0,40}\b(?:chang\w*|revis\w*|updat\w*|rewrit\w*|amend\w*|expan\w*|alter\w*|approv\w*|decisions?)|(?:outside|beyond) (?:the )?(?:ticket|contract|scope)|requires? (?:owner|human|host) (?:approval|decision|authorization)|(?:ask|request|need|require|obtain|seek|await|wait for|get)\b[^.!?\n]{0,100}\b(?:owner|human|host)\b[^.!?\n]{0,100}\b(?:approv\w*|authoriz\w*|decision|consent|permission|signoff|contract|policy|scope)|(?:change|expand|weaken|alter|revise|update|rewrite) (?:the )?(?:ticket|contract|policy|scope)|(?:grant|expose|retrieve) (?:host )?(?:credentials?|secrets?|permissions?))\b/iu.test(feedback);
}
