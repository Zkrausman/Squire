...
export interface ReportEvidencePort {
  write(content: string): Promise<ReportEvidence>;
...
export async function verifyReportEvidence(port: ReportEvidencePort, ref: ReportEvidence, content: string): Promise<void> {
  validateEvidenceRef(ref);
  const expected = Buffer.from(content, "utf8");
  const actual = await port.read(structuredClone(ref));
  if (!Buffer.isBuffer(actual) || actual.length > MAX_REPORT_BYTES || actual.length !== ref.byteLength || reportHash(actual) !== ref.sha256 || !actual.equals(expected)) throw new Error("report evidence bytes/content/length/digest mismatch");
}
function identity(s: BigIntStats): string { return `${s.dev}:${s.ino}:${s.size}:${s.mtimeNs}:${s.ctimeNs}`; }

/** Host-only evidence. Linux fd-relative operations pin the trusted directory;
 * other platforms fail closed until an equivalent safe-read adapter exists. */
