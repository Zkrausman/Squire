import { LAUNCH_CONDITIONS, type LaunchCondition } from "./launch-retry.js";
...
export const RUN_EVENT_TYPES = [
  "launch_reserved", "launch_dispatched", "launch_failed", "launch_returned",
...
export interface RunEvent {
  ...
  readonly launch?: { readonly id: string; readonly generation: 0 | 1; readonly classifierVersion: 1; readonly rule: LaunchCondition | null; readonly delayMs: number };
...
export function eventIdFor(fields: Pick<EventFields, "runId" | "ticketId" | "type"> & Partial<Pick<EventFields, "phase" | "attempt" | "outcome" | "correction" | "launch">>): string {
  const key = [
    ...
    ...(fields.launch ? [fields.launch.id] : []),
  ].join("|");
...
export function validateRunEvent(value: unknown): asserts value is RunEvent {
  ...
  if (typed.startsWith("launch_")) {
    const t = object["launch"] as RunEvent["launch"];
    if (!t || Object.keys(t).sort().join() !== "classifierVersion,delayMs,generation,id,rule" || !/^[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$/u.test(t.id) || ![0, 1].includes(t.generation) || t.classifierVersion !== 1 || (t.rule !== null && !LAUNCH_CONDITIONS.includes(t.rule)) || !Number.isSafeInteger(t.delayMs) || t.delayMs < 0 || t.delayMs > 14400000 || object["phase"] === undefined || object["attempt"] === undefined) throw new Error("invalid launch event");
  } else if (object["launch"] !== undefined) throw new Error("unexpected launch event");
...
export function deriveRunEvents(previous: PersonalRunState | undefined, next: PersonalRunState): readonly RunEvent[] {
  ...
  const events: RunEvent[] = [...launchEvents(next, previous?.launchTransitions?.length ?? 0), ...stagedEvents(next, previous?.stagedTransitions?.length ?? 0), ...correctionEvents(next, previous?.reportCorrections?.length ?? 0)];
...
export function synthesizeCurrentRunEvents(state: PersonalRunState): readonly RunEvent[] {
  ...
  const events: RunEvent[] = [...launchEvents(state), ...stagedEvents(state), ...correctionEvents(state, 0)];
...
function launchEvents(state: PersonalRunState, offset = 0): RunEvent[] {
  return (state.launchTransitions ?? []).slice(offset).map(t => createRunEvent({
    runId: state.runId, ticketId: state.ticketId, stateRevision: state.version, timestamp: new Date(t.timestamp).toISOString(),
    type: `launch_${t.kind}`, phase: t.phase, attempt: t.attempt,
    launch: { id: t.id, generation: t.number, classifierVersion: t.classifierVersion, rule: t.rule, delayMs: t.delayMs },
  }));
}
