...
export function eventIdFor(fields: Pick<EventFields, "runId" | "ticketId" | "type"> & Partial<Pick<EventFields, "phase" | "attempt" | "outcome" | "correction" | "generation">>): string {
  const key = [
    ...
    fields.outcome ?? "",
    ...(fields.generation ? [String(fields.generation)] : []),
...
export function validateRunEvent(value: unknown): asserts value is RunEvent {
  ...
  if (typed.startsWith("launch_")) {
    if (![1, 2].includes(object["generation"] as number) || object["phase"] === undefined || object["attempt"] === undefined) throw new Error("invalid launch event identity");
  ...
  const expected = eventIdFor({
    ...
    ...(object["generation"] === undefined ? {} : { generation: object["generation"] as 1 | 2 }),
...
function launchEvents(state: PersonalRunState, offset: number): RunEvent[] {
  return (state.launchGenerations ?? []).slice(offset).map(g => createRunEvent({ runId: state.runId, ticketId: state.ticketId, stateRevision: state.version, timestamp: g.timestamp, type: `launch_${g.kind}`, phase: g.phase, attempt: g.attempt, generation: g.generation }));
}
