...
export function piEvents(report: string, sessionId = fixtureSession, profile = fixtureProfile): any[] {
  ...
  return [{ type: "session", version: 3, id: sessionId, timestamp: "2026-01-01T00:00:00.000Z", cwd: "/private/workspace" }, { type: "agent_start" }, { type: "turn_start" }, { type: "message_start", message: user }, { type: "message_end", message: user }, { type: "message_start", message: { ...assistant, content: [] } }, { type: "message_end", message: assistant }, { type: "turn_end", message: assistant, toolResults: [] }, { type: "agent_end", messages: [user, assistant], willRetry: false }, { type: "agent_settled" }];
}
