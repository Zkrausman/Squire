...
export function powershell(script: string, ...args: string[]): string {
  ...
  const env = { ...process.env, SQUIRE_ACL_ARGS: JSON.stringify(args) };
  const source = `$ErrorActionPreference='Stop'; $ProgressPreference='SilentlyContinue'; $p=ConvertFrom-Json $env:SQUIRE_ACL_ARGS; ${script}`;
  return execFileSync("powershell.exe", ["-NoProfile", "-NonInteractive", "-EncodedCommand", Buffer.from(source, "utf16le").toString("base64")], { env, encoding: "utf8", timeout: 15_000 }).trim();
}
