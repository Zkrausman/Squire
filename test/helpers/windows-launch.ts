...
export function grant(file: string, sid: string, rights: string | number, inherit = false): void {
  powershell(`$a=Get-Acl -LiteralPath $p[0]; $s=[System.Security.Principal.SecurityIdentifier]::new($p[1]); $t=[System.Security.AccessControl.FileSystemRights]; if (-not ([Enum]::GetNames($t) -ccontains $p[2]) -and $p[2] -notmatch '^[0-9]+$') { throw 'Full FileSystemRights name or numeric value required' }; $r=[Enum]::Parse($t,$p[2],$false); $i=if($p[3] -eq 'true'){[System.Security.AccessControl.InheritanceFlags]'ContainerInherit,ObjectInherit'}else{[System.Security.AccessControl.InheritanceFlags]::None}; $a.AddAccessRule([System.Security.AccessControl.FileSystemAccessRule]::new($s,$r,$i,[System.Security.AccessControl.PropagationFlags]::None,[System.Security.AccessControl.AccessControlType]::Allow)); Set-Acl -LiteralPath $p[0] -AclObject $a`, file, sid, String(rights), String(inherit));
}
