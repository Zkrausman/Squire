...
export class FakePiProcess extends EventEmitter implements PiProcess {
  readonly identity: string; readonly stdout = new FakeStream(); readonly stderr = new FakeStream(); readonly writes: string[] = []; exitCode: number | null = null; thinkingLevel = "max";
  ...
  respondToLast(command: string, success = true, data?: unknown): void {
    const request = JSON.parse(this.writes.at(-1) ?? "{}") as { id?: string };
    const responseData = command === "get_state" && data && typeof data === "object" && !Array.isArray(data) && !("thinkingLevel" in data)
      ? { ...data, thinkingLevel: this.thinkingLevel }
      : data;
    this.send({ id: request.id, type: "response", command, success, ...(responseData === undefined ? {} : { data: responseData }) });
  }
}
export class FakePiProcessFactory implements PiProcessFactory {
  ...
  async spawn(spec: ProcessLaunch, signal?: AbortSignal, onSpawn?: (process: PiProcess) => void): Promise<FakePiProcess> { if (signal?.aborted) throw new Error("spawn aborted"); this.launches.push(spec); const process = new FakePiProcess(`process-${this.processes.length + 1}`); const thinkingAt = spec.args.indexOf("--thinking"); if (thinkingAt >= 0 && spec.args[thinkingAt + 1]) process.thinkingLevel = spec.args[thinkingAt + 1]!; this.processes.push(process); onSpawn?.(process); return process; }
}
