...
import { captureOwnerPiIdentity, requireOwnerModels, validateOwnerPiIdentity, verifyRunningParentPi, assertSandboxPiIdentity, type OwnerPiIdentity } from "../src/personal/runtime-parity.js";
...
test("running Pi acquisition rejects an unbound CLI instead of trusting an environment alias", async () => {
  await assert.rejects(captureOwnerPiIdentity(undefined, { getAvailable: () => [] }), /CLI path unavailable/);
  await assert.rejects(captureOwnerPiIdentity(process.execPath, { getAvailable: () => [] }), /running Pi package cannot be established/);
});
