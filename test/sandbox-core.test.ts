...
function binding(): GuestBinding { return { runId: "run_example01", sandboxName: deriveSandboxName("run_example01"), bootId: "boot-1", operationGeneration: 1, releaseId: "fixture-release", helperDigest: "f".repeat(64) }; }
...
class FakeWorker extends EventEmitter {
  readonly stdin = { write: (frame: Uint8Array): boolean => { const message = parseGuestMessage(frame.subarray(4)); const response: GuestOperationResponse = { schemaVersion: 1, kind: "squire-guest-operation-response", requestId: message.requestId, binding: message.binding, operation: message.operation, success: true, data: { ok: true } }; queueMicrotask(() => this.stdout.emit("data", encodeGuestFrame(response))); return true; } };
...
test("release resolution requires a validated signed exact v0.39.0 tuple", async () => {
  ...
  const selected = await resolver.resolve({ templateName: "registry/sandbox", templateDigest, resources, networkProfileDigest });
...
test("durable host supervisor records exact ownership and refuses an unhashed executable", async () => {
  const executable = process.execPath; const executableSha256 = sha256Bytes(await readFile(executable)); const supervisor = new HostProcessSupervisor({ ledger: new InMemoryHostProcessLedger() });
  const result = await supervisor.run({ kind: "version", executable, executableSha256, argv: ["-e", "process.exit(0)"], cwd: "/", environment: { PATH: "/usr/bin:/bin" } });
...
test("sandbox lifecycle creates, starts, retains, and removes only under the shared fence", async () => {
  ...
  const driver = new FakeSbxDriver(); const release = fakeSandboxRelease(); const resolver = new SandboxReleaseResolver({ releases: [release.release], verifySignature: () => true, observeBinary: observeFixtureBinary, platform: "linux", architecture: "x86_64" });
