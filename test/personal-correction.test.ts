...
test("explicit owner requests stop even under a mislabeled code-only recommendation",()=>{
 ...
 for(const finding of ["Ask the owner to approve a contract change before implementation","Owner approval is necessary for a contract amendment","Need host permission to proceed","Owner must decide whether to alter acceptance criteria","The contract needs revision before implementation","Contract must be updated before implementation","We must alter the contract","Host permissions are required to proceed","The change is beyond the ticket scope","Scope expansion is necessary"]){
