...
for (const stall of ["lookup", "reservation", "runtime", "runtime-record", "instructions", "spawn-intent", "spawn", "spawn-claim", "handshake", "validation", "registration"] as const) test(`first-session ${stall} stall is bounded and fenced`, async () => {
  ...
  const config = { roles: roleConfig, workspaceReadiness: testWorkspaceReadiness, materializer: deterministicMaterializer, processLeaseMs: 10, allocationStepTimeoutMs: 5, allocationTimeoutMs: 100 };
...
test("registered-session generation claim rejects a stale allocation owner", async () => {
  ...
  const factory = new RaceFactory("generation", gate); const resolver = new RaceResolver("generation", gate); const config = { roles: roleConfig, workspaceReadiness: testWorkspaceReadiness, materializer: deterministicMaterializer, processLeaseMs: 10, allocationStepTimeoutMs: 5, allocationTimeoutMs: 100 };
...
const postConfig = { roles: roleConfig, workspaceReadiness: testWorkspaceReadiness, materializer: deterministicMaterializer, processLeaseMs: 10, allocationStepTimeoutMs: 5, allocationTimeoutMs: 100, commandTimeoutMs: 100 };
