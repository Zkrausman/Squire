...
import type { ImplementPhaseResult, PersonalRunState, PlanPhaseResult, ReviewPhaseResult, TestPhaseResult } from "../src/personal/types.js";
...
function completedState(): PersonalRunState {
  ...
  return {
    ...
    baseSha: BASE,
    head: BASE,
    sessions: { plan: "plan-session", implement: "implement-session", review: "review-session", test: "test-session" },
    attempts: { plan: 1, implement: 1, review: 1, test: 1 },
    results: { plan: planResult(run.runId), implement: implementation, review, test: validation },
...
test("JSON run state is created exclusively and replaced by sequential versions", async () => {
  ...
  try {
    ...
    await store.save({ ...state(2), step: "plan", baseSha: BASE, head: BASE, attempts: { plan: 1, implement: 0, review: 0, test: 0 } });
    assert.equal((await store.read("aidev-1-0123456789"))?.step, "plan");
    assert.equal((await store.findActive("AIDEV-1"))?.version, 2);
    await assert.rejects(store.save({ ...state(4), step: "implement", baseSha: BASE, head: BASE, attempts: { plan: 1, implement: 1, review: 0, test: 0 } }), /advance by one/);
...
test("state validation rejects every mutable record boundary and identity mismatch", () => {
  const valid = { ...state(), step: "plan" as const, baseSha: BASE, head: BASE, attempts: { plan: 1, implement: 0, review: 0, test: 0 }, sessions: { plan: "plan-session" }, results: { plan: planResult(state().runId) } };
  validateState(valid);
...
test("completed state requires every latest phase and exact Review/Test input heads", () => {
