...
The controller is packaged as one service/CLI for the MVP. The sandbox template, controller, schema, and Pi/Herdr compatibility versions are released together. Template input is built with `scripts/build-sandbox-template.mjs`; promotion is host-only and requires the exact OCI digest/config/helper inventory, signed `sbx` v0.39.0 binary/help observations, and canonical evidence from the executable `scripts/acceptance/sandbox-host-worker.mjs` (the `sandbox-v039.mjs` entrypoint delegates to it). The worker creates the disposable sandbox, executes every destructive host probe itself, emits raw request-bound packets, and quarantines only the exact bridge after stop/remove. Windows execution uses ACL/reparse-point and PowerShell process primitives rather than POSIX mode/`/proc` claims. Missing worker capability or an unprovable disk tuple is a failure, never a skipped acceptance.
