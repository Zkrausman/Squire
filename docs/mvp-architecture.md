...
The trusted isolated Git component is specified in [AIDEV-222 Isolated Git Workspace](aidev-222-git-workspace.md). It owns the private bare repository, single linked worktree, bounded offline operations, immutable bundle handoff, retention metadata, and component disposal; downstream tickets retain sandbox, scheduling, host publication, and merge ownership. Its production constructor requires the runtime-authenticated filesystem-isolation authority composed by AIDEV-223: a private token is bound to the exact canonical ticket root, mount namespace, root identity, and observed mount topology, and the operation boundary fails closed when that evidence is stale or unavailable. AIDEV-223's sandbox boundary must prove same-filesystem bind-mount and path-swap resistance before AIDEV-222 can perform Git side effects. AIDEV-222's Node `st_dev` and descriptor checks are defense in depth, not a substitute for that proof.
