...
The controller is packaged as one service/CLI for the MVP. The sandbox template, controller, schema, and Pi/Herdr compatibility versions are released together. Template input is built with `scripts/build-sandbox-template.mjs`; promotion is host-only and requires the exact OCI digest/config/helper inventory, signed `sbx` v0.39.0 binary/help observations, and canonical evidence from `scripts/acceptance/sandbox-v039.mjs`. That worker consumes only strict observer request/result artifacts for destructive host probes and quarantines the exact bridge after stop/remove. Missing observer output or an unprovable disk tuple is a failure, never a skipped acceptance.
