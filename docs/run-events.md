...
State commits before the bounded outbox. Events describe reservation, launch generations, Implement/Verify start/completion, publication and terminal disposition. Schema version 2 permits only implement/verify phase labels and one logical attempt. Events contain no ticket prose, findings, prompts, secrets, URLs or log paths. IDs are deterministic and deduplicated.
