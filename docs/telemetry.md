...
## Stable handoff API and historical imports
...
[Historical cohorts](historical-cohorts.md) documents the separate explicit-run
backfill/import command, provisional session authority, authenticated Ed25519
CI/merge evidence and descriptive scorecards. The current-run authority above
is unchanged: historical sessions cannot be promoted into controller telemetry.
Comparisons keep unlike strata and incomplete accounting visible. Neither path
changes model-selection, remediation-budget, review, test or merge policy. The
narrowly scoped launch-retry policy is documented in personal-mvp.md.
