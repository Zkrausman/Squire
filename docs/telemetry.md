...
Phase reports are extracted from the last structured `agent_end` assistant's text, not from terminal presentation or transcript text. `agent_end` need not be the last stdout event: Pi 0.73.1 may emit a paired `compaction_start`/`compaction_end` with reason `threshold` after it. Both full-stream extraction and bounded transport preserve the exact terminal event across that pair, including an aborted/failed compaction with no result, provided `willRetry` is false. The compaction summary is never a phase report. Incomplete, malformed, duplicated or reordered trailing lifecycle events, unknown trailers, new agent work, and retry intent invalidate the old report selection rather than reusing stale evidence. Their existing strict report, HEAD, wiki, correction and independent-gate validation remains unchanged. Accounting validation is separate: bad usage can coexist with a valid phase result. Accounting-only malformed/duplicate members cannot repair or invalidate an otherwise unambiguous report. An absent/ambiguous/truncated terminal report still fails the existing handoff gate.

## Bounds, persistence and restart

Compaction invokes a separate model without authoritative assistant `message_end` usage on this stream. Any compaction event therefore marks session token accounting incomplete (`unsupported_compaction`); no summary text or `tokensBefore` value is used as usage. Valid post-report compaction cannot relabel a successful phase outcome.

- Each child stream has a 64 MiB exact-byte capture budget. The production transport **drains**, rather than kills, a child after accounting overflow. It separately bounds the current event and retained terminal event to 64 MiB each, so a later valid terminal report remains usable across supported trailing compaction. An invalid terminal selection is explicit and cannot fall back to an obsolete terminal in the captured prefix. A truncated accounting prefix is marked incomplete with `stream_limit`; its values are not counted. No child stderr or decoded event text is included in transport error messages.
