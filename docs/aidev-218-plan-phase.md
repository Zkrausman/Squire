...
The controller-owned Plan policy is appended after repository-configured instructions. The Plan command is run with Pi's fixed tool allowlist: controller-owned `squire_plan_read`, `squire_plan_grep`, `squire_plan_find`, `squire_plan_ls`, project-only `wiki_recall`, and `squire_submit_plan`. Pi's stock `read`, `grep`, `find`, and `ls` tools are deliberately disabled because they accept unrestricted absolute paths. `bash`, `edit`, `write`, package/install tools, wiki mutation tools, and arbitrary repository extensions are not enabled. The replacement tools accept repository-relative paths only, resolve through retained no-follow descriptors under the controller-derived workspace root, reject traversal/absolute paths, denied controller/sensitive paths, symlinks, hardlinks, special files, cross-device targets, oversize files, and changed-during-read identities, and return only relative result paths. Their policy digest and generated extension bytes are controller/materializer-bound. Wiki and footer extensions are loaded in that order around the digest-bound Plan extension; the footer remains last. Run-scoped `HOME` and `WIKI_HOME` are always used.
