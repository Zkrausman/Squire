...
First-session allocation is one bounded run/role protocol, not an unfenced sequence after lease acquisition. The runner renews and verifies a monotonic process-allocation token before and after runtime resolution, session lookup/validation, instruction loading, generation claim, spawn, handshake, and registration. Before the first spawn it persists a provisional `reserved → spawning → spawned` allocation. A new owner may replace only a stale `reserved` record; `spawning` or `spawned` fails closed because the external process outcome requires AIDEV-224 reconciliation. Spawn factories must honor cancellation without leaving a process alive, and first-session registration atomically verifies the current spawned identity/token and clears the allocation. Fake-clock races cover every pre-registration stall.
