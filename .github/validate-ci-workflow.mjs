...
exactKeys(tests, ["name", "timeout-minutes", "run"], "test step");
assert.equal(tests["timeout-minutes"], "5", "full test step must be bounded to 5 minutes");
...
assert.deepEqual(windowsLaunchJob, {
  ...
  steps: [
    ...
    { name: "Run Windows launch capture regression", "timeout-minutes": "5", run: "node --test --test-timeout=120000 dist/test/personal-windows-state-replace.test.js dist/test/personal-windows-launch.test.js dist/test/personal-launch-material.test.js dist/test/personal-background-status.test.js dist/test/personal-owner-observation.test.js dist/test/personal-controller.test.js dist/test/personal-plan-supervisor.test.js dist/test/personal-report-correction.test.js dist/test/personal-report-bytes.test.js dist/test/personal-windows-report-evidence.test.js dist/test/personal-windows-report-correction.test.js dist/test/personal-phase-transport.test.js" },
  ],
