...
- **Trust and routing.** Locate the approved Squire executable and its source/build
  identity, rather than trusting the first PATH hit. Squire requires Node.js 24
  only (`>=24 <25`) on host and sandbox. From the trusted checkout, run
  `node scripts/check-node-runtime.mjs`,
  `npm ci --engine-strict --ignore-scripts --no-audit --no-fund`, then
